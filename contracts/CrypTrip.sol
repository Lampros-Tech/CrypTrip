...
contract CrypTrip is NFT1 {
    ...
    mapping(uint => address) travelerIdToAddress;
    ...
    function addEndpoint(string memory _endpoint, uint _tier) public {
        tier[_tier] = _endpoint;
    }
    ...
    function bookHotel (uint _hotelId, uint _checkIn, uint _checkOut, uint _roomsTwoCount, uint _roomsThreeCount, uint _roomsFiveCount, uint _contactNumber) public payable {
        ...
        if(traveler[msg.sender].level == 0){
            travelersRegistered += 1;
            travelerIdToAddress[travelersRegistered] = msg.sender;
        }
        traveler[msg.sender].hotelsBooked += 1;
        traveler[msg.sender].level += 1;
        uint currentLevel = traveler[msg.sender].level;
        if (currentLevel % 5 == 0)
            traveler[msg.sender].tier = currentLevel / 5;
            safeMint(msg.sender, tier[traveler[msg.sender].tier]);
    }
    function getAllTravelers() public view returns (address _addr, uint _level){
        for (uint i = 1; i <= travelersRegistered; i++) {
            _addr = travelerIdToAddress[i];
            _level = traveler[travelerIdToAddress[i]].level;
        }
    }
}